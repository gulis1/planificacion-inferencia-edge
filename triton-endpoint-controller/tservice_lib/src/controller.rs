...
const FINALIZER_NAME: &str = "tritonservices.prueba.ucm.es/deletion";

/// This provides a hook for generating the CRD yaml (in crdgen.rs)
#[derive(CustomResource, Deserialize, Serialize, Clone, Debug, JsonSchema)]
#[cfg_attr(test, derive(Default))]
#[kube(kind = "TritonService", group = "prueba.ucm.es", version = "v1", namespaced)]
#[kube(status = "TritonServiceStatus", shortname = "tservice")]
pub struct TritoNodeSpec {
    pub selector: String
}
/// The status object of `Document`
#[derive(Deserialize, Serialize, Clone, Default, Debug, JsonSchema)]
pub struct TritonServiceStatus {
    pub prueba: i32
}
...
fn error_policy(_doc: Arc<TritonService>, error: &Error, _ctx: Arc<Context>) -> Action {
    error!("Error en reconcile: {}", error);
    Action::requeue(Duration::from_secs(5))
}

async fn reconcile(tservice: Arc<TritonService>, ctx: Arc<Context>, sender: mpsc::Sender<Message>) -> Result<Action> {
    
    let tservices = Api::<TritonService>::namespaced(ctx.client.clone(), tservice.metadata.namespace.as_ref().unwrap());
...
pub fn run(client: Client, sender: mpsc::Sender<Message>) {
    ...
    let tservices = Api::<TritonService>::all(client.clone());
